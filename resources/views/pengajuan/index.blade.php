...
<div class="w-full px-6 py-6 mx-auto">
    <div class="container mx-auto mt-6">
        ...
        <div class="flex justify-end -mb-px">
            <button id="diajukanBtn" onclick="setActiveTab('diajukan')" class="tab-button bg-white text-gray-800 font-bold py-2 px-6 rounded-t-lg shadow-md mr-2">Diajukan</button>
            <button id="riwayatBtn" onclick="setActiveTab('riwayat')" class="tab-button bg-gray-200 text-gray-400 font-bold py-2 px-6 rounded-t-lg border-b-0 mr-2">Riwayat</button>
        </div>

        <!-- Card Diajukan -->
        <div id="diajukanCard" class="tab-content bg-white shadow-md rounded-lg p-6 -mt-1 relative">
            <!-- Tombol Pengajuan -->
...
<script>
    ...
    function setActiveTab(card) {
        const url = new URL(window.location);
        url.searchParams.set('active_tab', card);
        window.history.pushState({}, '', url); // Perbarui URL tanpa refresh
        showCard(card);
    }
    ...
    window.addEventListener('load', function() {
        const urlParams = new URLSearchParams(window.location.search);
        const activeTab = urlParams.get('active_tab') || 'diajukan'; // Default ke 'diajukan' jika tidak ada
        // Pindah ke tab yang sesuai berdasarkan activeTab
        document.querySelector(`#${activeTab}`).classList.add('active'); // Misal tab punya id sesuai dengan activeTab
    });
